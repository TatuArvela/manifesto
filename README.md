...
- **Connected mode**: Client built with `VITE_MANIFESTO_SERVER=<url>` talks to a Manifesto server for storage, accounts, and live sync. The server picks a storage driver (`sqlite` or `postgres`) and an auth provider (`local` username/password or `oidc` SSO) independently, and all four combinations are supported.
